(() => {
  ...
  async function makeLocalLink(payload) {
    const encoded = await compressedEncode(payload),
      link = `${location.href.split('#')[0]}#lista-gz=${encoded}`;
  ...
  async function importLocalHash() {
    if (location.hash.startsWith('#lista-gz=')) {
      try {
        ...
        const changed = await importPayload(await compressedDecode(encoded));
  ...
  async function exportList() {
    ...
    try {
      const url = URL.createObjectURL(
          new Blob([JSON.stringify(await buildPayload(list), null, 2)], {
            type: 'application/json',
          })
        ),
        anchor = document.createElement('a');
      anchor.href = url;
      anchor.download = 'lista.json';
  ...
  async function shareList() {
    ...
    try {
      ...
      console.error(error);
      notify('Não foi possível gerar o compartilhamento.');
    }
  }
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(bind, 500);
    importRemote()
      .then((handled) => handled || importLocalHash())
      .catch((error) => {
        console.error(error);
        notify('Falha ao processar compartilhamento.');
      });
  });
