...
assert(
  source.includes('window.__mlDbIntegrityV230 = { diagnose, validateRows, report, runSilent }')
);
...
const baseRows = {
  catalogs: [
    { id: 'c1', name: 'Arroz', brand: 'Camil', unit: '5 kg', category: 'Alimentos', ean: '' },
  ],
  lists: [
    {
      id: 'l1',
...
assert.equal(
  validateRows(
    { ...baseRows, lists: [{ ...baseRows.lists[0], date: '2026-02-31' }] },
    { version: 6 }
  ).ok,
  false,
  ...
  validateRows(
    {
      ...baseRows,
      lists: [
        { ...baseRows.lists[0], items: [{ ...baseRows.lists[0].items[0], mainItemId: 'missing' }] },
      ],
    },
    { version: 6 }
  ).ok,
  false,
  'referência de item deve falhar'
);
assert.equal(
  validateRows(
    { ...baseRows, inventory: [{ ...baseRows.inventory[0], mainItemId: 'missing' }] },
    { version: 6 }
  ).ok,
  false,
  'referência de estoque deve falhar'
);
assert.equal(
  validateRows(
    {
      ...
      lists: [
        {
          ...baseRows.lists[0],
          items: [baseRows.lists[0].items[0], { ...baseRows.lists[0].items[0], id: 'i1' }],
        },
      ],
    },
    { version: 6 }
  ...
  false,
  'ID duplicado global deve falhar'
);
