...
assert(share.includes("delete x.inventory")&&share.includes("delete x.stock"),'estoque não pode entrar no compartilhamento');
assert(share.includes('shared-list-v2')&&share.includes('shared-list-v3'),'compatibilidade V2/V3 deve permanecer');
assert(backup.includes('MAX=20*1024*1024'),'limite de backup deve permanecer');
...
assert(inventory.includes('expiryDate')&&inventory.includes('minQuantity'),'estoque deve manter validade e estoque mínimo');
assert(list.includes('f.isConnected'),'formulário inválido não pode aplicar patch posterior');
...
assert(sw.includes('db-integrity-v230.js')&&sw.includes('db-migrations-v230.js'),'módulos de integridade/migração devem estar no SW');
