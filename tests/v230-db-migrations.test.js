...
function fakeDb(initial=[]){
  ...
  return {
    ...
    createObjectStore:(name,options)=>{names.add(name);return {name,keyPath:options.keyPath}},
    names
  };
}
...
assert.deepStrictEqual(Array.from(fresh.names),['catalogs','lists','history','settings','wishlist','trash','referenceProducts','referenceMarkets','inventory']);
assert.strictEqual(api.migrate(fresh,0,6),0,'migração repetida deve ser idempotente');

const v5=fakeDb(['catalogs','lists','history','settings','wishlist','trash','referenceProducts','referenceMarkets']);
