...
assert(source.includes('__mlShareOptimizedLegacyDisabled = true'), 'Compatibilidade legada deve estar desativada');
...
assert(!source.includes('accept="application/json,.json"'), 'Compatibilidade não deve solicitar JSON');
assert(shareConfig.includes("format: 'shared-list-v3-compact'"), 'Formato compacto deve pertencer ao fluxo oficial');
...
assert(sw.includes("'./share-optimized-v230.js'"), 'Service Worker deve carregar a compatibilidade');
