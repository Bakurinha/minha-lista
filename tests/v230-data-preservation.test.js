...
for(const key of ['catalogs','lists','history','wishlist','trash','inventory','settings'])assert.deepStrictEqual(JSON.parse(JSON.stringify(migrated[key])),original[key],`${key} foi alterado durante migração`);
...
assert.deepStrictEqual(JSON.parse(JSON.stringify(legacyMigrated.inventory)),[]);
