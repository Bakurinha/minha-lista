...
assert.deepStrictEqual(migrated,original,'migração V2 deve ser identidade e preservar todos os campos');
...
for(const key of ['catalogs','lists','history','wishlist','trash','inventory','settings'])assert.deepStrictEqual(restoredMigrated[key],original[key],`${key} foi alterado durante o round-trip`);
...
const cases=[
  ...
  ['item órfão',()=>{const bad=JSON.parse(JSON.stringify(original));bad.lists[0].items[0].mainItemId='missing-catalog';return api.validate(bad)}],
  ['data impossível',()=>{const bad=JSON.parse(JSON.stringify(original));bad.lists[0].date='2026-02-31';return api.validate(bad)}]
];
