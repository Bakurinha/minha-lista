...
assert.match(app, /DB_NAME\s*=\s*['"]MinhaListaDB['"][\s\S]*?DB_VERSION\s*=\s*6/, 'app.js must use IndexedDB V6');
assert.match(app, /objectStoreNames\.contains\('inventory'\)/, 'app.js must create inventory during upgrade');
assert.match(inv, /const\s+DB\s*=\s*['"]MinhaListaDB['"][\s\S]*?VERSION\s*=\s*6,[\s\S]*?STORE\s*=\s*['"]inventory['"]/, 'inventory module must target V6');
...
const productsExpr = inv.match(/const\s+PRODUCTS\s*=\s*(\[[\s\S]*?\n\s*\]);\s*const\s+UNITS\s*=/)?.[1];
const unitsExpr = inv.match(/const\s+UNITS\s*=\s*(\{[\s\S]*?\n\s*\});\s*const\s+MARKET_NAMES\s*=/)?.[1];
const marketsExpr = inv.match(/const\s+MARKET_NAMES\s*=\s*(\[[\s\S]*?\n\s*\]);\s*function\s+referenceSeed/)?.[1];
