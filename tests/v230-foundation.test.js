...
assert.match(inv,/const DB='MinhaListaDB', VERSION=6, STORE='inventory'/,'inventory module must target V6');
...
const productCount=products.reduce((n,p)=>n+(units[p[2]]||['1 un']).slice(0,7).length,0);
...
assert(!/inventory/.test(fs.readFileSync('enhancements.js','utf8')),'existing sharing layer must not reference inventory in stage 1');
console.log(`V2.3.0 foundation OK: ${productCount} products, ${new Set(markets).size} markets, DB V6/inventory upgrade present.`);
