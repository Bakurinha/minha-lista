...
(() => {
  ...
  async function all(store) {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        request.onerror = () => finish(reject, request.error || Error('Falha na leitura'));
      } catch (error) {
        finish(reject, error);
      }
    });
  ...
  async function createShare(list) {
    ...
    const response = await fetch(`${API}/api/share`, {
      ...
      body: JSON.stringify(cleanPayload(list, await all('catalogs'))),
      cache: 'no-store',
    });
  ...
  async function importCode(code, askConfirmation = true) {
    ...
    if (!CODE_RE.test(normalizedCode)) {
      notify('Código inválido. Use os 12 caracteres do compartilhamento.');
      return false;
    }
    if (!API) {
    ...
    try {
      const response = await fetch(`${API}/api/share/${encodeURIComponent(normalizedCode)}`, {
        cache: 'no-store',
      ...
      const items = payload.list.items.map((item) => {
        ...
        const copyItem = { ...item, id: uid(), mainItemId };
        delete copyItem.inventory;
        delete copyItem.stock;
        return copyItem;
      });
      ...
      await writeImported(newList, newCatalogs);
      notify('Lista importada com sucesso.');
  ...
  async function consumeIncomingCode() {
    ...
    window.__mlShareIncomingHandled = true;
    history.replaceState(null, '', location.pathname);
    await importCode(sid);
  }
