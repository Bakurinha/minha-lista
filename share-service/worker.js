...
function cleanCatalog(c) {
  ...
  const cid = text(c.id, 200),
    ...
    ean = typeof c.ean === 'string' ? c.ean.replace(/\D/g, '') : '';
  if (!cid || !name || !name.trim() || ean.length > 14) return null;
  return {
    id: cid,
    name: name.trim(),
    brand: text(c.brand, 120) || '',
    unit: text(c.unit, 60) || 'un',
    category: text(c.category, 80) || 'Outros',
...
function cleanItem(i) {
  ...
  const iid = text(i.id, 200),
    mainItemId = text(i.mainItemId, 200),
    date = dateOrNull(i.date),
  ...
  if (
    ...
    !finite(i.quantity, 1000000) ||
    !finite(i.value, 1000000000) ||
    !finite(i.packageQuantity, 1000000)
  )
    return null;
  const packageUnit =
    i.packageUnit === undefined || i.packageUnit === null || i.packageUnit === ''
      ? ''
      : text(i.packageUnit, 60);
  if (packageUnit === null) return null;
  return {
    id: iid,
    mainItemId,
    done: typeof i.done === 'boolean' ? i.done : Boolean(i.done),
    quantity: i.quantity ?? null,
    date,
    value: i.value ?? null,
    marketName: text(i.marketName, 160) || '',
    comments: text(i.comments, 2000) || '',
    productName: text(i.productName, 120) || '',
    productBrand: text(i.productBrand, 120) || '',
    productUnit: text(i.productUnit, 60) || 'un',
    productCategory: text(i.productCategory, 80) || 'Outros',
    packageQuantity: i.packageQuantity ?? null,
    packageUnit,
    expiryDate,
  };
}
export function validatePayload(payload) {
  ...
  const name = text(list.name, 120),
    listDate = dateOrNull(list.date);
  if (!name || !name.trim() || listDate === undefined) return { ok: false, error: 'list.name' };
  if (!Array.isArray(payload.catalogs) || payload.catalogs.length > MAX_CATALOGS)
    return { ok: false, error: 'catalogs' };
  if (!Array.isArray(list.items) || list.items.length > MAX_ITEMS)
    return { ok: false, error: 'items' };
  const catalogs = payload.catalogs.map(cleanCatalog);
  if (catalogs.some((x) => !x)) return { ok: false, error: 'catalog.item' };
  const ids = new Set(catalogs.map((x) => x.id));
  if (ids.size !== catalogs.length) return { ok: false, error: 'catalog.duplicate-id' };
  const items = list.items.map(cleanItem);
  if (items.some((x) => !x || !ids.has(x.mainItemId)))
    return { ok: false, error: 'item.reference' };
  return {
    ok: true,
    value: {
      ...
      list: {
        ...
        comments: text(list.comments, 2000) || '',
        archived: false,
        items,
      },
      catalogs,
...
async function createShare(request, env, origin) {
  ...
  try {
    ...
    return json(
      { error: e.message === 'body-too-large' ? 'payload-too-large' : 'invalid-json' },
      400,
      origin
    );
  }
  const checked = validatePayload(payload);
  if (!checked.ok) return json({ error: 'invalid-payload', field: checked.error }, 400, origin);
  const id = randomId();
  await env.SHARES.put(id, JSON.stringify(checked.value), { expirationTtl: SHARE_TTL });
  return json(
    { id, url: `${new URL(request.url).origin}/s/${id}`, expiresIn: SHARE_TTL },
    201,
    origin
  );
}
